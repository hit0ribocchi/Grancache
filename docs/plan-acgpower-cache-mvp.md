...
| 上游（本机代理端口 → 节点 → 游戏服） | 响应头 + 响应体（可能是 gzip，也可能 chunked） |
...
| 项 | 说明 | 建议 |
|---|---|---|
| 真机未验证 / exe 未重打包 | 目前所有结论都来自离线测试，浏览器→代理→上游的真实链路未跑 | 经用户同意后 `--stop` → 打包 → 按 §3.4 清单验证；打包前留档旧 exe |
...
- 效果：太郎直接请求**能解析的真实域名** —— **代理开着正常、代理停着也正常**（直接走本机代理软件的规则出网）；
